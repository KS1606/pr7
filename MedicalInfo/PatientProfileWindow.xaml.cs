...
using MedicalInfo.MedicalSystemDataSetTableAdapters; // Подключаем адаптеры
...
namespace MedicalInfo
{
    public partial class PatientProfileWindow : Window
    {
        ...
        public PatientProfileWindow(int UserId)
        {
            InitializeComponent();
            this.userId = UserId;
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MedicalHistoryPage detailWindow = new MedicalHistoryPage(selectedMedicalHistory.Id);
                detailWindow.Show();
            }
            else
            {
        ...
        {
            // Открытие окна записи на прием (например, новое окно для выбора времени)
